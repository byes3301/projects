...
namespace Game_FinalCamera
{
	class Camera
	{
		...
		public:
			...
			void changeCamera(glm::vec3 cameraPos, int cameraState, float offsetX, float offsetY)
			{
				
				Set(glm::vec3(1+offsetX, offsetY, 3), glm::vec3(1+offsetX, 0.5 + offsetY, 0), glm::vec3(0, 1, 0));
			
			}
			void moveCamera(int cameraState, int offsetX, int offsetY) {
				if (cameraState == 0) {
					TranslateForward((float)offsetX);
					TranslateUpword((float)offsetY);
				}
			}
